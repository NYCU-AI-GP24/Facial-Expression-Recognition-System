    ...
    with torch.no_grad():
        while True:
            ...
            for idx, (x, y, w, h) in enumerate(faces):
                ...
                emo_input = emo_preprocess(
                    Image.fromarray(cv2.cvtColor(crop_bgr, cv2.COLOR_BGR2RGB))).unsqueeze(0).to(DEVICE)
                rgb_input = rgb_preprocess(
                ...
                txt = f"{emo_label} | {race_label} | {gender_label}"
                cv2.putText(frame, txt, (x, y-10),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2)
            cv2.namedWindow('Emotion | Race | Gender', cv2.WINDOW_NORMAL)
            cv2.resizeWindow('Emotion | Race | Gender', 1280, 720)
            cv2.imshow('Emotion | Race | Gender', frame)
